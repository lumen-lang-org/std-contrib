...
export function handleConnection(socket: Socket, onMessage: (peer: Peer, message: string) => void): void {
  ...
  while (true) {
    ...
    // behind the socket.
    while (true) {
      let frame = decodeFrame(buffer, MAX_MESSAGE);
      if (frame.error != "") {
        closePeer(peer, CLOSE_PROTOCOL_ERROR, frame.error);
        return;
      }
      ...
      if (assembly.error != "") {
        closePeer(peer, CLOSE_PROTOCOL_ERROR, assembly.error);
        return;
      }
      ...
      if (assembly.opcode == OP_CLOSE) {
        ...
        let code = closeCodeOf(assembly.message);
        closePeer(peer, code, "");
        return;
      }
      ...
      onMessage(peer, assembly.message);
    }
