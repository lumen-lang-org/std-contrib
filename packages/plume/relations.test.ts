...
//
// Runs against SQLite; relations_pg.test.ts and relations_mysql.test.ts are
// this file with one import changed, because "all three nest the same way" is
// a claim only three runs can make.
//
//   sh packages/plume/build.sh
//   cd packages/plume && lumen test relations.test.ts
...
function connectionConfig(): DbConfig {
  let named: DbConfig = { filename: "/tmp/plume_relations_test.db" };
  return named;
}
