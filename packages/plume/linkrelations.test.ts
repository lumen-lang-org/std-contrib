...
//
//   cd packages/plume && lumen test linkrelations.test.ts
...
function agentsRepo(): DbRepository {
  let rs: DbRelation[] = [
    hasManyThrough({
      ...
      table: "lk_servers",
      foreignColumn: "id",
      linkTable: "lk_agent_servers",
      linkLocalColumn: "agent_id",
      linkForeignColumn: "server_id",
      localColumn: "id",
      columns: "id, server_name AS \"serverName\", url",
    }),
    hasManyThrough({
      field: "subAgents",
      table: "lk_agents",
      foreignColumn: "id",
      linkTable: "lk_agent_children",
      linkLocalColumn: "parent_id",
      linkForeignColumn: "child_id",
      localColumn: "id",
...
function seeded(): DbRepository {
  let cfg: DbConfig = { filename: "/tmp/plume_linkrel_test.db" };
  ...
  execute(database, "CREATE TABLE lk_agent_servers (agent_id text NOT NULL, server_id text NOT NULL)");
  execute(database, "CREATE TABLE lk_agent_children (parent_id text NOT NULL, child_id text NOT NULL)");
...
test("an unsafe name anywhere in the join refuses the relation", () => {
  expect(!relationValid(hasManyThrough({
    field: "s",
    table: "lk_servers",
    foreignColumn: "id",
    linkTable: "x; DROP TABLE y",
    linkLocalColumn: "agent_id",
    linkForeignColumn: "server_id",
    localColumn: "id",
    columns: "id",
