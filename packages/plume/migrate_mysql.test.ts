...
//   cd packages/plume && lumen test migrate_mysql.test.ts
//
// The same suite as migrate.test.ts, against MySQL. The migration bodies are
// raw SQL, so they are spelled MySQL's way — `varchar(64)` where the others
...
function basePlan(): Migration[] {
  let plan: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY)"),
    migration("2", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
  ];
  return plan;
}
...
test("a failing migration names itself and stops the rest", () => {
  ...
  let plan: Migration[] = [
    ...
    migration("3", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
  ];
...
test("a description containing a backslash is recorded, not lost", () => {
  ...
  clean();
  let plan: Migration[] = [
    migration("1", "note C:\\path\\", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY)"),
  ];
  let first = migrate(database, plan);
  expect(first.ok);
...
test("editing an applied migration is refused, not ignored", () => {
  ...
  let edited: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY, extra text)"),
    migration("2", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
  ];
...
test("repair accepts an edit that was deliberate", () => {
  ...
  expect(migrate(database, basePlan()).ok);
  let edited: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY) -- a comment"),
    migration("2", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
  ];
...
test("a migration inserted below one already applied is refused by default", () => {
  ...
  let merged: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY)"),
    migration("1.5", "from another branch", "CREATE TABLE mig_b_extra (id varchar(64))"),
    migration("2", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
  ];
...
test("a repeatable step re-runs when its statement changes and not otherwise", () => {
  ...
  let v1: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY, name text)"),
  ...
  let v2: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY, name text)"),
    repeatable("mig_view", "CREATE OR REPLACE VIEW mig_view AS SELECT id, name FROM mig_a"),
  ];
...
test("baseline marks history without running anything", () => {
  ...
  let plan: Migration[] = [
    migration("1", "create a", "CREATE TABLE mig_a (id varchar(64) PRIMARY KEY)"),
    migration("2", "create b", "CREATE TABLE mig_b (id varchar(64) PRIMARY KEY)"),
    migration("3", "create c", "CREATE TABLE mig_c (id varchar(64) PRIMARY KEY)"),
  ];
