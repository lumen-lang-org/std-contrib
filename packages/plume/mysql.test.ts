// The same suite as plume.test.ts, against MySQL instead of PostgreSQL.
//
...
//   cd packages/plume && lumen test mysql.test.ts
...
import { mysql, mysqlConnection, mysqlVersion } from "./mysql.ts";
...
test("two connections each hold their own rows", () => {
  let repo = seeded();
  let writer = mysqlConnection(testConfig());
  let reader = mysqlConnection(testConfig());
...
test("a config naming nothing to reach is refused, not attempted", () => {
  let nothing: DbConfig = { user: "root" };
  let spare = mysqlConnection(nothing);
