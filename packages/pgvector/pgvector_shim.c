// A scalar/string-friendly C surface over libpq, for pgvector.
//
...
//
// Build:
//   cc -c pgvector_shim.c -I$(pg_config --includedir) -o pgvector_shim.o
...
int pgv_connect(const char *conninfo) {
    clear_result();
    if (g_conn) {
        PQfinish(g_conn);
        g_conn = NULL;
...
int pgv_query0(const char *sql) {
    return exec_params(sql, 0, NULL);
}

int pgv_query1(const char *sql, const char *a) {
    const char *vals[1] = { a };
    return exec_params(sql, 1, vals);
}

int pgv_query2(const char *sql, const char *a, const char *b) {
    const char *vals[2] = { a, b };
    return exec_params(sql, 2, vals);
}

int pgv_query3(const char *sql, const char *a, const char *b, const char *c) {
    const char *vals[3] = { a, b, c };
    return exec_params(sql, 3, vals);
}

int pgv_query4(const char *sql, const char *a, const char *b, const char *c, const char *d) {
    const char *vals[4] = { a, b, c, d };
    return exec_params(sql, 4, vals);
}

int pgv_query5(const char *sql, const char *a, const char *b, const char *c, const char *d, const char *e) {
...
const char *pgv_value(int row, int col) {
    if (!g_res) return "";
...
const char *pgv_error(void) {
    return g_error;
}

const char *pgv_version(void) {
    static char buf[64];
    if (!g_conn) return "";
