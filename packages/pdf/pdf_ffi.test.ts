// The FFI binding, against PDFs written by the test itself.
//
...
import { extractText, extractLayout, extractPage, extractPageLayout, readInfo, pageCount, popplerVersion } from "./pdf_ffi.ts";

const PDF_DIR = "/tmp/lumen-pdf-ffi-test";
...
test("a page beyond the end is reported with the real count", () => {
  seedPdfs();
  let r = extractPage(PDF_DIR + "/three.pdf", 9);
  expect(!r.ok);
  expect(r.error.indexOf("out of range") >= 0);
  expect(r.error.indexOf("has 3") >= 0);
});

test("the library reports its version", () => {
  expect(popplerVersion().length > 0);
});

test("a page reads with layout preserved", () => {
  seedPdfs();
  let r = extractPageLayout(PDF_DIR + "/three.pdf", 1);
  expect(r.ok);
  expect(r.text.indexOf("page one alpha") >= 0);
});
...
test("dates are zero when the document carries none", () => {
  seedPdfs();
  let info = readInfo(PDF_DIR + "/one.pdf");
  expect(info.ok);
  expect(info.created == 0);
