// /admin with no tab named — the same page, opened on Agents.
//
// A separate file rather than an optional parameter: LumenJS resolves a route
// by file, and a bracketed segment does not match the empty string, so
// without this /admin itself is a 404 while /admin/agents works. Everything
// here is the sibling file with the parameter absent.
...
const TAB_OF: Record<string, string> = {
  ...
  "sign-in": "Sign-in",
  "tracing": "Tracing",
  // Banner and Search were missing, so /admin/banner and /admin/search both
  // silently opened Models — a link that looks like it works and lands
  // somewhere else. Every admin-zone tab in src/settings.ts belongs here; the
