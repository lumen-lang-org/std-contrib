...
function fresh(): SecretService {
  ...
  return new SecretService(database, testKey());
}

function stored(keys: SecretService, name: string, owner: string): string {
  let made = keys.mint({
...
test("a stored secret lists by name and opens only through the runner's door", () => {
  let keys = fresh();
  let id = stored(keys, "stripe key", "o1");
  let listed = keys.listing("o1");
  ...
  let row = keys.repository.byId(id, "o1");
  expect(row.header == "Authorization");
  expect(keys.value(row) == "Bearer sk-test-0001");
});

test("somebody else's secret is absent, not forbidden", () => {
  let keys = fresh();
  let id = stored(keys, "stripe key", "o1");
  expect(keys.repository.byId(id, "o2").id == "");
  expect(keys.repository.byName("stripe key", "o2").id == "");
  expect(keys.listing("o2") == "[]");
  expect(!keys.forget(id, "o2"));
  expect(keys.repository.byId(id, "o1").id == id);
});
...
test("deleting a secret takes its envelope with it", () => {
  ...
  expect(keys.forget(id, "o1"));
  expect(!hasCredential(database, "secret:" + id));
  expect(keys.repository.byId(id, "o1").id == "");
});

test("what a secret refuses at the door", () => {
  let keys = fresh();
  expect(keys.mint({ owner: "o1", name: "", value: "v",
    destination: "https://a.example", header: "", category: "", master: testKey(), now: "t" }).fault != "");
  expect(keys.mint({ owner: "o1", name: "k", value: "v",
  ...
  expect(keys.mint({ owner: "o1", name: "k", value: "",
    destination: "https://a.example", header: "", category: "", master: testKey(), now: "t" }).fault != "");
  stored(keys, "stripe key", "o1");
  expect(keys.mint({ owner: "o1", name: "Stripe Key", value: "v",
...
test("the per-owner bound refuses the twenty-first", () => {
  ...
  while (i < MAX_SECRETS_PER_OWNER) {
    stored(keys, "key " + `${i}`, "o1");
    i = i + 1;
  }
  let over = keys.mint({ owner: "o1", name: "one more", value: "v",
...
test("a graph may only aim a secret at the address it was stored for", () => {
  let keys = fresh();
  let id = stored(keys, "stripe key", "o1");
  expect(keys.repository.graphFault(graphWith(httpNode("https://api.example.com/v2/charges", id)), "o1") == "");
  let moved = keys.repository.graphFault(graphWith(httpNode("https://evil.example/steal", id)), "o1");
  ...
  expect(keys.repository.graphFault(graphWith(httpNode("https://{{prev}}/x", id)), "o1") != "");
  expect(keys.repository.graphFault(graphWith(httpNode("https://api.example.com/v2", id)), "o2") != "");
  expect(keys.repository.graphFault(graphWith(httpNode("https://anywhere.example/x", "")), "o1") == "");
});

test("a use is stamped, so the list can say what is alive", () => {
  let keys = fresh();
  let id = stored(keys, "stripe key", "o1");
  expect(keys.repository.byId(id, "o1").lastUsedAt == "");
  keys.touch(id, "t9");
  expect(keys.repository.byId(id, "o1").lastUsedAt == "t9");
});

test("a step may carry several secrets, and the spelling it used to carry still works", () => {
  let keys = fresh();
  let one = stored(keys, "stripe key", "o1");
  let made = keys.mint({ owner: "o1", name: "trace key", value: "Bearer t-2",
  ...
  expect(keys.repository.graphFault(graphWith(both), "o1") == "");
  expect(secretIds(both).length == 2);

  let away = httpNode("https://elsewhere.example/v2", one + "," + two);
  expect(keys.repository.graphFault(graphWith(away), "o1") != "");
  ...
  expect(keys.repository.graphFault(graphWith(old), "o1") == "");
  ...
  expect(keys.repository.graphFault(graphWith(elsewhere), "o1") == "");
});
