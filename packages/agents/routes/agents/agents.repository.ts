...
export class AgentRepository {
  ...
  one(id: string): string { return findById(this.db, this.full, id); }

  exists(id: string): bool { return existsById(this.db, this.flat, id); }

  hasModelConfig(id: string): bool { return existsById(this.db, modelConfigsMapping(this.db), id); }

  hasPrompt(id: string): bool { return existsById(this.db, promptsMapping(), id); }

  hasServer(id: string): bool { return existsById(this.db, mcpServersMapping(), id); }

  hasSkill(id: string): bool { return existsById(this.db, skillsMapping(), id); }

  save(document: string): DbResult { return persist(this.db, this.flat, document); }

  // Only one agent is the default, so claiming it takes it from whoever held it.
  clearDefaults(): DbResult { return setEvery(this.db, this.flat, "is_default", "0"); }
  ...
  scopes(id: string): string[] { return agentScopes(this.db, id); }

  grant(id: string, scope: string): string { return grantScope(this.db, id, scope); }

  revoke(id: string, scope: string): string { return revokeScope(this.db, id, scope); }

  embeddingUsable(modelId: string): bool { return embeddingModel(this.db, modelId).id != ""; }

  saveRetrieval(row: AgentRetrievalRow): DbResult {
    return persist(this.db, agentRetrievalMapping(), JSON.stringify(row));
  }

  retrieval(id: string): string { return findById(this.db, agentRetrievalMapping(), id); }

  webRag(id: string): AgentWebRagRow { return webRagFor(this.db, id); }

  saveWebRag(row: AgentWebRagRow): DbResult {
    return persist(this.db, agentWebRagMapping(), JSON.stringify(row));
  }

  storedWebRag(id: string): string { return findById(this.db, agentWebRagMapping(), id); }

  model(id: string): string { return findById(this.db, modelsMapping(), id); }

  runs(id: string, tags: string[], limit: int): string { return runsOf(this.db, id, tags, limit); }
