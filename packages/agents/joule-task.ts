...
export function joulePrompt(task: string): string {
  return task.trim()
    ...
    + " Run one with no arguments to read its spec. Build the file with the command and"
    + " keep that JSON spec as the one source of truth: to change the result, edit the"
    + " spec and run the command again to the same path. Do not hand-edit the output with"
    + " python-pptx or python-docx, and do not re-open the file to check it — the"
    + " command's own \"wrote ...\" line is the confirmation. Writing the library by hand,"
    + " tweaking the output afterwards, or re-rendering to verify is where turns go."
    + " fetch-image is how a picture comes from the web:"
